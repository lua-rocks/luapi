...
local colors = require 'warnings.ansicolors'
...
local function parseUniversal(block, path, api, name, order)
  ...
  -- remove empty tables
  if next(api[name].params) == nil then api[name].params = nil end
  if next(api[name].returns) == nil then api[name].returns = nil end
end
